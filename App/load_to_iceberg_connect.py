...
def load_data_via_connect(csv_path):
    ...
    # 클라이언트는 복잡한 설정 없이 서버 주소만 필요합니다.
    # 모든 Iceberg/HMS/S3A 설정은 이미 docker-compose의 spark-connect 서버가 들고 있습니다.
    spark = (
    ...
    # Spark Connect에서 로컬 파일을 직접 spark.read.csv로 읽으려면 서버 컨테이너 안에
    # 파일이 있어야 하므로, 클라이언트의 파일을 Pandas로 먼저 읽는 것이 가장 확실합니다.
    ...
    # HMS의 default warehouse (file:/...)를 따라가지 않습니다.
    spark.sql(
